...
@section('content')
    ...
    <div id="page-wrapper">
        <div class="row">
            <div class="col-lg-12">
                <h3 class="page-header">Thêm phòng ban</h3>
            </div>
        </div>
        <center>
            <div class="frame">
                    ...
                    @endif
                        <form action="them" method="POST">
                            ...
                            <div class="form-group input-group">
                                ...
                                <input class="form-control" name="txtten" placeholder="Nhập tên phòng ban"
                                       required/>
                            </div>
                            <div class="form-group input-group">
                                <span class="input-group-addon" rows="4">Mô tả</span>
                                <textarea class="form-control" rows="4" name="txtmota" placeholder="Nhập mô tả phòng ban"
                                          required></textarea>
                            </div>
                            <div class="form-group input-group">
                                <span class="input-group-addon">Loại yêu cầu</span>
                                <select class="form-control" name = "loaiyeucau">
                                    @foreach($requirementtypeModel as $emp)
                                        <option
                                                value="{{$emp->RT_ID}}">{{$emp->RT_NAME}}</option>
                                    @endforeach
                                </select>
                            </div>
